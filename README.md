...
- **目标会话只接受飞书输入**：DSH 界面里手打的、别的自动化塞进来的用户消息**一律不处理**（那一轮以 `blocked` 结束），避免同一条会话有两个驱动源——谁都不知道哪一轮的产出算谁的，还会和"等这一轮跑完"打架。被并进同一轮时只丢外来那条，飞书请求照样拿到回答；工具结果、技能注入、运行时上下文都不算"人在说话"。想让界面也能往里说话：补丁层写 `lockInput: false`
...
- 目标会话**只接受飞书输入**：在 DSH 界面里直接往这条会话发消息不会得到回应（那一轮以 `blocked` 结束），要看日志才知道；这是有意的，见「行为」里的说明
