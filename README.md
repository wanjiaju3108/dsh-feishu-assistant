...
- **回答走一张流式卡片**：文本消息不渲染 Markdown，所以回答用卡片回；而且**一条回答只用一张卡**——先发一张写着「正在处理」的卡，之后每段把累计正文全量传给飞书。飞书客户端保留共同前缀、把**新增的那部分逐字打出来**（第一段也不例外），打字速度由卡片里的 `streaming_config` 写死（默认 30ms 打 2 个字）；正文超过卡片上限（30 KB）会自动**翻页**再接一张卡；整轮结束关掉流式模式
...
- **目标会话一旦指定，就不再接受别处的输入**：DSH 界面里手打的、别的自动化塞进来的用户消息**一律不处理**（那一轮以 `blocked` 结束），避免同一条会话有两个驱动源。被并进同一轮时只丢外来那条，飞书请求照样拿到回答；工具结果、技能注入、运行时上下文都不算"人在说话"。**只拦"人在说话"**——界面里切模型/推理深度（走 `model/selection` 事件）、停止（走 `agent.cancel`）、斜杠命令（走 `command/run`）都不经过拦截点，照常可用。想让界面也能往里说话：补丁层写 `lockInput: false`
...
- id: feishu-assistant
  ...
  config:
    ...
    managerId: ''
    persona: ''
    # 目标会话是否只接受飞书输入；想让 DSH 界面也能往里说话就设成 false
    lockInput: true
```
...
- 目标会话**不接受别处的输入**：在 DSH 界面里直接往这条会话发消息不会得到回应（那一轮以 `blocked` 结束），要看日志才知道；界面仍然可以看，也能切模型/停止，只是不能往里说话。这是有意的，见「行为」里的说明
...
- 收尾前会按最后一段的字数**等一会儿再关流式**（每字 15ms，最多 4 秒），否则那一关会把还没打出来的内容一次性刷出来；这段等待占用请求队列，所以长回答之后的**下一条**请求最多晚 4 秒开始
- 没有发送者限流；没有 `/` 命令
