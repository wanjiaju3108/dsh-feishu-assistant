 ...
 * @param deps.openAnswerCard 开一张流式回答卡片（初始正文就是「正在处理」）
 * @param deps.enqueue 审批通过后把请求交给请求队列
 * @returns 审批流程句柄
 */
export function createApprovalFlow({
  ...
  /**
   * 管理员同意：给请求人开一张流式回答卡片（初始正文就是「处理中」），带着它进请求队列。
   *
   * 回答卡片开不出来（没权限、接口失败）时退回一条普通「处理中」回复，行为跟以前一致。
   *
   * @param item 待审批请求
   */
  async function approveRequest(item) {
    const answerCard = await openAnswerCard({
      messageId: item.messageId,
      initialText: REPLY_TEXT.processing,
    });
    if (!answerCard) {
      await sendReply(item.messageId, REPLY_TEXT.processing).catch((error) => {
        logger.warn(`回复处理中提示失败：${errorMessage(error)}`);
      });
    }
    enqueue(item.messageId, item.content, answerCard);
  ...
  function handleCardAction(event) {
    ...
    if (value.action === 'approve') {
      // 开回答卡片要发请求，别拖住卡片回调的响应（飞书要求 3 秒内回执）。
      void approveRequest(item).catch((error) => {
