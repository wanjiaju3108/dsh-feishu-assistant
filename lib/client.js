  ...
  factory: (require) => {
    ...
    const MESSAGES = {
      zh: {
        ...
        'hint.persona': '选一份本地 md/txt 上传，内容会存进插件自己的配置（$DSH_HOME/settings.yaml）并立刻生效。插件不会改动你的原文件；之后你改了原文件，要重新上传一次。上限 64 KB。',
        'button.upload': '上传人设文件…',
        'import.tooLarge': '这个文件超过 64 KB，没有上传',
        'import.empty': '这个文件是空的，没有上传',
        ...
        'status.personaImported': '已导入 {name}（{bytes} / 65536 字节）',
        'status.personaNone': '还没有上传人设文件（飞书AI助理模式未启用）',
      ...
      en: {
        ...
        'hint.persona': 'Upload a local md/txt file. Its text is stored in the plugin\'s own config ($DSH_HOME/settings.yaml) and takes effect immediately. Your original file is never modified; if you edit it later, upload again. Cap: 64 KB.',
        'button.upload': 'Upload persona file…',
        'import.tooLarge': 'That file is larger than 64 KB; nothing uploaded',
        'import.empty': 'That file is empty; nothing uploaded',
        ...
        'status.personaImported': 'imported {name} ({bytes} / 65536 bytes)',
        'status.personaNone': 'no persona file uploaded yet (mode off)',
    ...
    function createSection(t) {
      ...
      return function Section() {
        ...
        /**
         * 上传一份人设文件：浏览器读内容（读不到路径，也不需要），直接交给宿主保存。
         *
         * 上传即生效，没有"先填进输入框再点保存"这一步；文件名一并带上，状态区要靠它
         * 说明现在生效的是哪一份。
         *
         * @param event 文件输入框的 change 事件
         */
        async function uploadPersonaFile(event) {
          ...
          try {
            ...
            const response = await fetch(CONFIG_ROUTE, {
              ...
              body: JSON.stringify({ persona: text, personaName: file.name }),
            });
            const payload = await response.json();
            if (!response.ok) throw new Error(payload?.error ?? `HTTP ${response.status}`);
            setSnapshot(payload);
        ...
        /**
         * 当前目标会话在状态区显示成什么：优先会话名，读不到名字就退回会话 ID。
         *
         * @returns 会话名；列表里没有这个会话、或它还没有名字时，退回会话 ID
         */
        function currentSessionLabel() {
          const found = sessions.find((session) => session.id === snapshot?.sessionId);
          return found?.title || snapshot?.sessionId || '';
        }
        ...
        return h('section', { style: styles.section }, [
            ...
            h('div', { key: 'persona', style: styles.row }, [
              ...
              h('div', { key: 'f', style: styles.field }, [
                h('input', {
                  ...
                  style: { display: 'none' },
                  onChange: (event) => void uploadPersonaFile(event),
                }),
                h('div', { key: 'r', style: styles.pickRow }, [
                  ...
                  }, uploading ? t('button.saving') : t('button.upload')),
                ]),
          ...
          h(StatusRow, {
            ...
            value: snapshot?.sessionId
              ? `${currentSessionLabel()} · ${snapshot.sessionLive ? t('session.live') : t('session.notLive')}`
              : t('status.none'),
          }),
          ...
          h(StatusRow, {
            ...
            value: snapshot?.personaActive
              ? t('status.personaImported')
                .replace('{bytes}', String(snapshot?.personaBytes ?? 0))
                .replace('{name}', snapshot?.personaName || '(unnamed)')
