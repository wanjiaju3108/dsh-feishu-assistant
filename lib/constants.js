...
/** 回答卡片里那个富文本元素的 ID；流式更新要按它定位。 */
export const ANSWER_CARD_ELEMENT_ID = 'answer';

/** 回答卡片的卡片 JSON 字节上限；飞书硬上限 30KB，留点余量。 */
export const ANSWER_CARD_MAX_BYTES = 24 * 1024;

/**
 * 回答卡片的打印速率：每 ANSWER_CARD_PRINT_FREQUENCY_MS 毫秒打 ANSWER_CARD_PRINT_STEP 个字。
 *
 * 不显式写就得猜客户端默认值（实测比这个慢，最后一段常被收尾打断）；写死了才好算"该等多久"。
 */
export const ANSWER_CARD_PRINT_FREQUENCY_MS = 30;

/** 每个打印间隔打几个字；配合上面的间隔决定打字速度。 */
export const ANSWER_CARD_PRINT_STEP = 2;

/**
 * 收尾前最多等多久，让客户端把最后一段打完。
 *
 * 关掉流式模式会把还没打出来的内容一次性刷出来（"打几个字突然整段蹦出"），所以要等；
 * 但等太久会一直占着"生成中"、卡片也不能转发，所以封顶，超出的部分认了。
 */
export const ANSWER_CARD_TYPING_GRACE_MAX_MS = 4000;
...
/** 会话里用户消息落盘的事件类型；插件靠它对上自己注入的那条，判定这一轮归谁。 */
export const USER_MESSAGE_EVENT_TYPE = 'user/message';
