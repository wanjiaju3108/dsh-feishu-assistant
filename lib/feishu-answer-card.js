...
export function createAnswerCardFlow({ logger, send }) {
   ...
   * @returns 发送目标；两个都没给时 undefined
   */
  function sendTarget(messageId, openId) {
    ...
    const sent = target.messageId
      ...
      : await send(
        (client) => client.im.message.create({
          ...
          data: { receive_id: target.openId, msg_type: INTERACTIVE_MESSAGE_TYPE, content },
        }),
        describe,
      );
    ...
    function sendTail(target, text) {
      ...
      return send(
        target.messageId
          ? (client) => client.im.message.reply({
            path: { message_id: target.messageId },
            data: { msg_type: TEXT_MESSAGE_TYPE, content },
          })
          : (client) => client.im.message.create({
            params: { receive_id_type: 'open_id' },
            data: { receive_id: target.openId, msg_type: TEXT_MESSAGE_TYPE, content },
          }),
        '补发回答卡片遗漏内容',
      );
    }
