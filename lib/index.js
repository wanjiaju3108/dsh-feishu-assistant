...
import { createPersonaInjector, loadPersona } from './persona.js';
import { pickPersonaFile } from './file-picker.js';
...
export async function apply(ctx, config) {
  ...
  const state = {
    ...
    /** 人设文件的读取结果：读不出来时是人话原因，读出来了就是空串。 */
    personaError: '',
    /** 当前生效的人设文本；空串表示「飞书AI助理模式」没开。 */
    personaText: '',
    connected: false,
    lastError: '',
    sessionError: '',
  ...
  /** 人设注入器：把用户写的人设文件挂到目标会话上。 */
  const persona = createPersonaInjector({ logger });

  /**
   * 重新读一遍人设文件，把结果记进状态。
   *
   * 每次读都是实时的：用户改完文件不用重启，下一条消息就是新的人设。
   *
   * @returns 读取结果（含 ok / path / text 或 error）
   */
  async function refreshPersona() {
    const loaded = await loadPersona(state.personaFile);
    state.personaError = loaded.ok ? '' : loaded.error;
    state.personaText = loaded.ok ? loaded.text : '';
    return loaded;
  }

  /**
   * 按当前人设同步目标会话：文件有效就挂上，失效就撤掉。
   *
   * @param agent 目标会话的 agent；拿不到时只刷新状态
   * @returns 挂上或本来就是这一份时为 true
   */
  async function syncPersona(agent) {
    const loaded = await refreshPersona();
    if (!loaded.ok) {
      persona.release(state.sessionId);
      return false;
    }
    return persona.sync(agent, state.sessionId, loaded.text);
  }
  ...
  try {
    ...
    if (typeof stored?.personaFile === 'string') state.personaFile = stored.personaFile;
  } catch (error) {
  ...
  async function injectToSession(text) {
    ...
    // 文件失效时 mode 就是没开，注入进去只会让人设凭空消失，不如直接拦住。
    if (!(await syncPersona(agent))) {
      logger.warn(`人设文件不可用（${state.personaError}），本条消息被丢弃`);
      return false;
    }
    agent.followup({
      id: randomUUID(),
  ...
  if (settingsScope) {
    ctx.on('settings/updated', (namespace, next) => {
      ...
      state.personaFile = next?.personaFile ?? '';
  ...
  async function buildSnapshot() {
    ...
    // 顺手读一遍人设文件：状态页显示的是此刻的真实情况，不是上一次保存时的。
    ...
    return {
      ...
      personaActive: state.personaText.length > 0,
      personaChars: state.personaText.length,
  ...
  /**
   * 弹出宿主机的系统文件选择框，把选中的绝对路径回给设置页。
   *
   * 设置页在浏览器里跑，拿不到宿主机路径，所以这一步只能在宿主机上做。这里只回路径，
   * 落盘还是靠用户在设置页点保存。
   *
   * @param body 请求体，可带 current（当前路径，用来定初始目录）
   * @returns { path } | { cancelled: true } | { error }
   */
  async function pickPersonaFileForSetting(body) {
    const current = typeof body?.current === 'string' ? body.current : state.personaFile;
    return pickPersonaFile({ current });
  }

  /**
   * 保存设置页的普通配置。
   *
   * 没带的字段沿用当前值：设置页漏传时不能把另一项清掉。人设文件读不出来不算保存
   * 失败——路径先存着、错误照实回报即可，用户可以先写路径再写文件。
   *
   * @param body 请求体
   */
  async function saveConfig(body) {
    ...
    const managerId = typeof body?.managerId === 'string'
      ? body.managerId.trim()
      : (current.managerId ?? '');
    const personaFile = typeof body?.personaFile === 'string'
      ? body.personaFile.trim()
      : (current.personaFile ?? '');
    if (settingsScope) await settingsScope.replace({ sessionId, managerId, personaFile });
    const previousSessionId = state.sessionId;
    state.sessionId = sessionId;
    state.managerId = managerId;
    state.personaFile = personaFile;
   ...
   *
   * 人设文件没配好，「飞书AI助理模式」就是没开：管理员得到一句指路，其他人静默丢弃，
  ...
  if (webServer) {
    registerSettingRoutes({
      ...
      actions: {
        ...
        saveCredentials,
        startPairing,
        pickFile: pickPersonaFileForSetting,
      },
    });
  }
