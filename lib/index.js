...
export async function apply(ctx, config) {
  ...
  async function handleRequest(messageId, text, fromManager) {
    ...
    // 先把等待挂上再回"正在处理"：不然回执发出去的这点时间可能把 turn/end 错过。
    const turnEnding = opener.waitForTurnEnd(state.sessionId);
    // 管理员直发这条路径此前没有任何回执，长回答期间飞书一片静默，分不清在跑还是死了。
    if (fromManager) await outbound.replyTo(messageId, REPLY_TEXT.processing);
    const turnEnd = await turnEnding;
    // 这一轮结束了却一个字都没产出（例如跑到一半出错）：不能静默，给请求人一个交代。
    if (repliedInCurrentRequest) return;
  ...
  // 会话事件回写：assistant 消息一段一段回飞书；agent 反问时给一条提示。
  ctx.on('session/event', (session, event) => {
    ...
    if (event?.type === TOOL_CALL_EVENT_TYPE) announceAskQuestion(event.data);
  });
