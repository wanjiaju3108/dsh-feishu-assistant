...
import { createInputLock } from './input-lock.js';
...
export const Config = z.object({
  ...
  persona: z.string().default(''),
  personaName: z.string().default(''),
  // 目标会话只接受飞书输入（见 input-lock.js）。想让 DSH 界面也能往里说话就设成 false。
...
export async function apply(ctx, config) {
  ...
  const approvalFlow = createApprovalFlow({
    ...
    enqueue: (messageId, text, card) => {
      void requestQueue.push(() => handleRequest(messageId, text, false, card)).catch((error) => {
  ...
  const opener = createAgentOpener({
    ...
    hasOwnTurn: () => routing.requestTurnSeen,
    setError: (message) => {
  ...
  async function handleRequest(messageId, text, fromManager, providedCard) {
    beginRequest(routing, { messageId, card: providedCard });
    ...
    if (!routing.answerCard) {
      ...
      routing.answerCard = await outbound.openAnswerCard({
        messageId,
        initialText: REPLY_TEXT.processing,
      });
      // 开不出来就退回一条普通回执，行为跟以前一致。
      if (!routing.answerCard) await outbound.replyTo(messageId, REPLY_TEXT.processing);
    }
  ...
  // 目标会话只接受飞书输入：界面里手打的、别的自动化塞的用户消息一律不处理（见 input-lock.js）。
  if (config?.lockInput ?? true) {
    const inputLock = createInputLock({
      ctx,
      logger,
      routing,
      getSessionId: () => state.sessionId,
