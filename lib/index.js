...
export async function apply(ctx, config) {
   ...
   * 私聊发一条文本给管理员。
   *
   * 和 sendReply 的区别：这是主动发起的新消息，不是回复某条已有消息。
   *
   * @param text 消息正文
   */
  async function notifyManager(text) {
    if (!restClient || !state.managerId) return;
    await restClient.im.message.create({
      params: { receive_id_type: 'open_id' },
      data: {
        receive_id: state.managerId,
  ...
  async function handleRequest(messageId, text, fromManager) {
    ...
    if (!(await injectToSession(text))) {
      ...
      await sendReply(messageId, fromManager ? REPLY_TEXT.sessionUnavailable : REPLY_TEXT.failure);
      return;
    }
