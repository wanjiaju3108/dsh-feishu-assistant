...
export async function apply(ctx, config) {
  ...
  const approvalFlow = createApprovalFlow({
    ...
    enqueue: (messageId, text, card) => {
      void enqueueRequest(messageId, text, false, card);
    },
  });
   ...
   * @param providedCard 审批流程已经开好的卡片；没有时才自己开
   * @returns Promise
   */
  async function enqueueRequest(messageId, text, fromManager, providedCard) {
    const card = providedCard ?? await outbound.openAnswerCard({
      messageId,
