 ...
 * @param deps.openAnswerCard 开一张回答卡片（私聊目标）
 * @param deps.sendText 卡片开不出来时退回的普通私聊文本
 * @param deps.getManagerId 取管理员 open_id；取不到就不推
 * @returns 推送器：push 推一段正文，finish 收尾
 */
export function createExternalAnswer({ logger, openAnswerCard, sendText, getManagerId }) {
  /**
   * 推一段正文给管理员。
   *
  ...
  function push(routing, text) {
    ...
    // 开卡是异步的，这里不等它；但它内部的 await 万一抛错，不能让异常无声无息地跑掉。
    void open(routing, managerId).catch((error) => {
      routing.externalOpening = false;
      logger.warn(`推送外部轮次的回答失败：${String(error)}`);
    });
  }
