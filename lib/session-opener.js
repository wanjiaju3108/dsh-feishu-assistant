 ...
 * agent（没有就主动 resume）、等它真空下来、等它把一轮 turn 跑完。
 *
 ...
 * @param deps.hasPendingInjection 注入的消息是不是还没被某一轮取走
 * @returns 打开器句柄
 */
export function createAgentOpener({ ctx, agents, logger, getSessionId, setError, hasPendingInjection }) {
  /**
   ...
   *
   * **这里不认轮次**——判定"这一轮是自己的"，靠的是调用方的前置动作，两条前提缺一不可：
   *
   * 1. 注入前 `waitUntilIdle()` 等会话空下来，所以注入进去的那条就是下一轮；
   * 2. `inbox.claim()` 对 `next-turn` **一次只取一条**，且 `next-turn` 没有插队（prepend）路径，
   *    所以它必然被第一个取走。
   *
   * 谁要改"先注入、再等空闲"的顺序，这两条就不成立了：别的来源那一轮的 turn/end 会提前把请求放行
   * （请求人收到"没有产出回答"，真回答走成私聊卡片）。真发生了这里会留一条告警日志。
   *
   * turn/end 也可能送不到：事件是先写会话日志再派发的，日志写不进去（例如目录被删）这一条就发不出来。
  ...
  function nextTurnEnd(sessionId) {
    return new Promise((resolve) => {
      ...
      const dispose = ctx.on('session/event', (session, event) => {
        ...
        // 注入的那条还没被取走，就说明这条 turn/end 不是自己的（前提被破坏了）：行为不变，留个线索。
        if (hasPendingInjection()) {
          logger.warn('这条 turn/end 不是飞书请求自己的（注入的消息还没被取走），请求可能提前收尾');
        }
        finish(event.data);
      });
  ...
  /**
   * 等目标会话真空下来：没有正在跑的轮次，也没有排队的输入。
   *
   * `whenIdle()` 在驱动把队列排空、落回 idle 时 resolve——只要 inbox 里还有消息，agent 一直报
   * `running`，所以"空闲"本身就意味着排空了；多看一眼 inbox 是为了堵住"刚空闲就被人插一条"的缝：
   * 那一瞬间插进来的消息会被驱动接走，我们回去接着等。
   *
   * 飞书请求注入前等它，是为了不和别处的轮次交错：会话忙的时候注入，我们那条会排在别人后面，
   * 而"这一轮跑完了"的判定（见 waitForTurnEnd）会被别人那一轮的 turn/end 提前满足。
   *
   * @param agent 目标会话的 agent
   * @returns 会话空下来时 resolve
   */
  async function waitUntilIdle(agent) {
    for (;;) {
      await agent.whenIdle();
      const pending = (agent.inbox?.nextTurn?.length ?? 0) + (agent.inbox?.nextStep?.length ?? 0);
      if (pending === 0) return;
    }
  }

  return { ensureAgent, waitUntilIdle, waitForTurnEnd: nextTurnEnd };
}
