 ...
 * @param deps.hasOwnTurn 本插件自己注入的那一轮开始了没有
 * @returns 打开器句柄
 */
export function createAgentOpener({ ctx, agents, logger, getSessionId, setError, hasOwnTurn }) {
  /**
   ...
   * 两个前提：
   *
   * - **认轮次**：请求刚注入时可能别的来源那一轮还在跑，只有 `hasOwnTurn()` 置上之后的 turn/end 才算；
   * - **可能收不到**：事件是先写会话日志再派发的，日志写不进去（例如目录被删）这一条就发不出来，
   *   所以额外巡检两层兜底，避免队列被一条永远等不到 turn/end 的请求卡死：
   *   agent 从注册表里没了（会话被关掉）、agent 从 running 回到 idle（跑完了但事件没送到）。
   *
  ...
  function nextTurnEnd(sessionId) {
    return new Promise((resolve) => {
      ...
      const dispose = ctx.on('session/event', (session, event) => {
        ...
        // 认轮次：自己那一轮还没开始时的 turn/end（DSH 界面/别的自动化那一轮）不算处理完，
        // 否则请求会被提前放行——请求人收到"没有产出"，真正的回答却走成了私聊卡片。
        if (!hasOwnTurn()) return;
        finish(event.data);
      });
